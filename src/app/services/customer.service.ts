...
import { Customer, CustomerQueryParams , CustomerListResponse } from '../models/customer.model';
...
export class CustomerService {
  ...
  registerCustomer(request: Customer) {
  ...
  getCustomerList(filter?: { name?: string; page?: number; limit?: number }) {
    ...
    return this.http.get(`${environment.apiBaseUrl}/customer/all`, { 
      params,
      responseType: 'text'
    }).pipe(
      map(response => {
