...
import { Configuration, RequestConfiguration, ConfigurationListResponse } from '../models/config.model';
...
export class ConfigurationService {
  ...
  registerConfig(request: RequestConfiguration) {
    return this.http.post(`${environment.apiBaseUrl}/config/`, request);
  }

  getConfigList(filter?: { code?: string; name?: string; page?: number; limit?: number }) {
    ...
    return this.http.get(`${environment.apiBaseUrl}/config/all`, { 
      params,
      responseType: 'text'
    }).pipe(
      map(response => {
  ...
  getConfigByID(id: number) {
    return this.http.get(`${environment.apiBaseUrl}/config/${id}`);
  }

  updateConfig(id: number, request: RequestConfiguration) {
    return this.http.post(`${environment.apiBaseUrl}/config/`, request);
  }

  deleteConfigByID(id: number) {
    return this.http.delete(`${environment.apiBaseUrl}/config/${id}`);
  }
}
