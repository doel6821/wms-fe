...
export class ReceiveOrderService {
  ...
  getReceiveOrderList(filter?: { supplierId?: number; page?: number; limit?: number, dueDate?: string, startDate?: string, endDate?: string, paymentStatus?: string }) {
  ...
  paymentReceiveInvoice(receiveId: number, request: PaymentReceiveRequest) {
      return this.http.post(`${environment.apiBaseUrl}/finance/payment`, request , { 
                responseType: 'text'
              }).pipe(
                map(response => {
                  try {
